...
func getTransactionHashesFromDatabase(collection collections.Collection, dbInstance *mongo.Database) (map[string][]*transactionInput, error) {
	...
	results := make(map[string][]*transactionInput)
	for _, blockchain := range cltInfo.Blockchain {
		tLogsCollection := database.CollectionInstance(dbInstance, &TransactionLog{})
		logsExistingHashes, err := tLogsCollection.Distinct(context.Background(), "transaction_hash", bson.M{"blockchain": blockchain})
		if err != nil {
			return nil, err
		}
		...
		infExistingHashes, err := tInfoCollection.Distinct(context.Background(), "transaction_hash", bson.M{"blockchain": blockchain})
		if err != nil {
			return nil, err
		}
		...
		for cursor.Next(context.Background()) {
			...
			result := &transactionInput{
				txHash:    txHash,
				fetchLogs: true,
				fetchInfo: !slices.Contains(infExistingHashesStr, txHash.TransactionHash),
			}
